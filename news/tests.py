...
class ModelTest(TestCase):
    def setUp(self):
        Event.objects.create(id=4,name='one plus',status=True,limit=2000,
                             address='shenzhen',start_time='2017-03-31 00:00:00')
        Guest.objects.create(id=4,event_id=2,realname='allen',phone="1122",
                             email='alen@mail.com', sign=False)
    def test_event_models(self):
            result= Event.objects.get(name='one plus')
            self.assertEqual(result.address,"shenzhen")
            self.assertTrue(result.status)

    def test_guest_models(self):
            result= Guest.objects.get(phone="1122")
...
class LoginActionTest(TestCase):
    ...
    def test_login_action_username_password_null(self):
        ''' 用户名密码为空 '''
        response = self.client.post('/news/login_action/', {'username': '', 'password': ''})
        self.assertEqual(response.status_code, 200)
        self.assertIn(b"username or password error!", response.content)

    def test_login_action_username_password_error(self):
        ''' 用户名密码错误 '''
        response = self.client.post('/news/login_action/', {'username': 'abc', 'password': '123'})
        self.assertEqual(response.status_code, 200)
        self.assertIn(b"username or password error!", response.content)

    def test_login_action_success(self):
        ''' 登录成功 '''
        response = self.client.post('/news/login_action/', data={'username': 'admin', 'password': 'admin123456'})
...
class EventMangeTest(TestCase):
    ...
    def setUp(self):
        ...
        self.client.post('/news/login_action/', data=login_user)  # 预先登录
    ...
    def test_event_mange_success(self):
        ''' 测试发布会:xiaomi5 '''
        response = self.client.post('/news/event_manage/')
        self.assertEqual(response.status_code, 200)
    ...
    def test_event_mange_search_success(self):
        ''' 测试发布会搜索 '''
        response = self.client.post('/news/search_name/', {"name": "xiaomi5"})
        self.assertEqual(response.status_code, 200)
        self.assertIn(b"xiaomi5", response.content)
        self.assertIn(b"beijing", response.content)
...
class GuestManageTest(TestCase):
    ...
    def test_event_mange_success(self):
        ''' 测试嘉宾信息: alen '''
        response = self.client.post('/news/guest_manage/')
        self.assertEqual(response.status_code, 200)
...
class SignIndexActionTest(TestCase):
    ...
    def setUp(self):
        ...
        login_user = {'username': 'admin', 'password': 'admin123456'}
        self.client.post('/news/login_action/', data=login_user)

    def test_sign_index_action_phone_null(self):
        ''' 手机号为空 '''
        response = self.client.post('/news/sign_index_action/1/', {"phone": ""})
        self.assertEqual(response.status_code, 200)
        self.assertIn(b"phone error.", response.content)

    def test_sign_index_action_phone_or_event_id_error(self):
        ''' 手机号或发布会id错误 '''
        response = self.client.post('/news/sign_index_action/2/', {"phone": "18611001100"})
        self.assertEqual(response.status_code, 200)
        self.assertIn(b"event id or phone error.", response.content)

    def test_sign_index_action_user_sign_has(self):
        ''' 用户已签到 '''
        response = self.client.post('/news/sign_index_action/2/', {"phone": "18611001101"})
        self.assertEqual(response.status_code, 200)
        self.assertIn(b"user has sign in.", response.content)

    def test_sign_index_action_sign_success(self):
        ''' 签到成功 '''
        response = self.client.post('/news/sign_index_action/1/', {"phone": "18611001100"})
        self.assertEqual(response.status_code, 200)
